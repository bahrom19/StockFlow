...
export class SupplierContactsRepository {
  ...
  async create(
    ...
    try {
      ...
      return await this.getClient(tx).supplierContact.create({ data });
    } catch (error: unknown) {
      // Prisma P2002: partial unique index violation — another active
      // primary contact already exists for this supplier (concurrent
      // assignment). Map to ConflictException; other errors propagate.
      const err = error as { code?: string };
      if (err?.code === 'P2002') {
        throw new ConflictException(
          'Another primary contact already exists for this supplier',
        );
      }
      throw error;
    }
  ...
  async update(
    ...
    try {
      ...
      if (rowVersion !== undefined) {
        ...
        return (await client.supplierContact.findUnique({
          where: { id },
        })) as unknown as SupplierContact;
      }

      return await client.supplierContact.update({
        where: { id },
        data,
      });
    } catch (error: unknown) {
