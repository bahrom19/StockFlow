...
describe('Billing Commercial Integration', () => {
  ...
  const mockPlan = { id: 'plan-1', code: 'starter', name: 'Starter', priceMonthly: 29.99, priceYearly: 290.00, currency: 'USD', trialDays: 14, maxUsers: 3, maxWarehouses: 1, maxProducts: 500, featureFlags: {}, isActive: true, sortOrder: 1, rowVersion: 0, createdAt: new Date(), updatedAt: new Date(), deletedAt: null };
  const mockSubscription: any = { id: 'sub-1', companyId: 'comp-1', planId: 'plan-1', status: 'TRIAL', trialStartsAt: new Date(), trialEndsAt: new Date(Date.now() + 14 * 86400000), currentPeriodStart: new Date(), currentPeriodEnd: new Date(Date.now() + 14 * 86400000), cancelledAt: null, cancelReason: null, cancelAtPeriodEnd: false, pastDueAt: null, suspendedAt: null, willExpireAt: null, paymentRetryCount: 0, lastPaymentAttempt: null, providerCustomerId: null, providerSubscriptionId: null, isActive: true, rowVersion: 0, createdAt: new Date(), updatedAt: new Date(), deletedAt: null, plan: mockPlan };
  const mockInvoice: any = { id: 'inv-1', companyId: 'comp-1', subscriptionId: 'sub-1', invoiceNumber: 'INV-TEST-0001', status: 'PENDING', subtotal: 29.99, discountAmount: 0, taxAmount: 0, totalAmount: 29.99, paidAmount: 0, currency: 'USD', dueDate: new Date(Date.now() + 30 * 86400000), paidAt: null, providerInvoiceId: null, notes: null, rowVersion: 0, createdAt: new Date(), updatedAt: new Date(), deletedAt: null };
  ...
  beforeEach(async () => {
    ...
    planRepo = {
      create: jest.fn(), findById: jest.fn(), findByCode: jest.fn(), findAll: jest.fn(),
      update: jest.fn(), softDelete: jest.fn(), upsertByCode: jest.fn(),
    } as any;

    subRepo = {
      create: jest.fn(), findByCompany: jest.fn(), findById: jest.fn(), findAll: jest.fn(),
      updateByCompany: jest.fn(), updateStatus: jest.fn(),
      findExpiredTrials: jest.fn(), findExpiringToday: jest.fn(),
      findOverdueGracePeriod: jest.fn(), findExpiredSuspensions: jest.fn(),
      findPendingRetries: jest.fn(),
    } as any;

    invRepo = {
      create: jest.fn(), findById: jest.fn(), findBySubscription: jest.fn(),
      findAll: jest.fn(), update: jest.fn(), softDelete: jest.fn(),
      getNextInvoiceNumber: jest.fn(),
    } as any;

    pmtRepo = { create: jest.fn(), findById: jest.fn(), findByInvoice: jest.fn().mockResolvedValue([]), update: jest.fn() } as any;
    usageRepo = { upsert: jest.fn(), getCurrentUsage: jest.fn(), findByCompany: jest.fn(), resetMetric: jest.fn() } as any;

    mockPrisma = {
      $transaction: jest.fn((cb: any) => cb(mockTx)),
      invoice: { findFirst: jest.fn().mockResolvedValue(null), findUnique: jest.fn().mockResolvedValue(null) },
      companySubscription: { findFirst: jest.fn().mockResolvedValue(null), findMany: jest.fn().mockResolvedValue([]), findUnique: jest.fn().mockResolvedValue(null), update: jest.fn().mockResolvedValue({}) },
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SubscriptionPlanService, CompanySubscriptionService, InvoiceService, StripeProvider, WebhookEngineService,
  ...
  describe('Trial → Checkout → Active', () => {
    it('1a. creates trial subscription', async () => {
      ...
      const sub = await subscriptionService.create('comp-1', { planCode: 'starter' }, 'user-1');
    ...
    it('1b. creates checkout session via StripeProvider', async () => {
      const session = await stripeProvider.createCheckoutSession({
        companyId: 'comp-1', customerId: 'cus_1', customerEmail: 't@t.com',
        customerName: 'T', planCode: 'starter', planName: 'Starter',
        priceAmount: '29.99', currency: 'USD', trialDays: 14,
        successUrl: 'https://x.com/success', cancelUrl: 'https://x.com/cancel',
      });
    ...
    it('1c. handles checkout.session.completed webhook', async () => {
      ...
      const result = await webhookEngine.handleWebhook({
        ...
        data: { object: { id: 'cs_1', customer: 'cus_1', subscription: 'sub_1', metadata: { companyId: 'comp-1', planCode: 'starter' } } },
        created: Date.now(),
      } as any);
  ...
  describe('Invoice → Payment → Receipt', () => {
    it('2a. generates invoice for subscription', async () => {
      subRepo.findById.mockResolvedValue({ ...mockSubscription, plan: mockPlan } as any);
      invRepo.getNextInvoiceNumber.mockResolvedValue('INV-TEST-0001');
      invRepo.create.mockResolvedValue({ ...mockInvoice, lines: [] } as any);
      const inv = await invoiceService.generateInvoice('sub-1', 'comp-1', 'user-1');
    ...
    it('2b. marks invoice as paid + creates PaymentTransaction + AuditLog', async () => {
      ...
      invRepo.update.mockResolvedValue({ ...mockInvoice, status: 'PAID' } as any);
  ...
  describe('Payment Failure → Retry', () => {
    it('3a. creates refund via StripeProvider', async () => {
      const refund = await stripeProvider.createRefund({ paymentIntentId: 'pi_1', amount: '29.99', reason: 'request' });
    ...
    it('3b. handles charge.refunded webhook', async () => {
      const result = await webhookEngine.handleWebhook({
        ...
        data: { object: { payment_intent: 'pi_1', amount_refunded: 2999, currency: 'usd' } },
        created: Date.now(),
      } as any);
      expect(result.handled).toBe(true);
    });
  ...
  describe('Subscription Lifecycle', () => {
    it('4a. cancels active subscription', async () => {
      subRepo.findByCompany.mockResolvedValue(activeSub() as any);
      subRepo.updateByCompany.mockResolvedValue({ ...activeSub(), status: 'CANCELLED' } as any);
    ...
    it('4b. resumes cancelled subscription', async () => {
      ...
      subRepo.updateByCompany.mockResolvedValue({ ...cancelledSub(), status: 'ACTIVE' } as any);
  ...
  describe('Webhook Idempotency', () => {
    it('5a. skips duplicate webhook events', async () => {
      const payload: any = { id: 'evt_dup', type: 'checkout.session.completed', data: { object: { id: 'cs_dup', metadata: { companyId: 'comp-1', planCode: 'starter' } } }, created: Date.now() };
  ...
  describe('Billing Portal', () => {
    it('6a. creates billing portal session', async () => {
      const portal = await stripeProvider.createBillingPortalSession({ customerId: 'cus_1', returnUrl: 'https://x.com/billing' });
  ...
  describe('Unsupported Events', () => {
    it('7a. skips unknown event types', async () => {
      const result = await webhookEngine.handleWebhook({ id: 'evt_x', type: 'unknown.event', data: { object: {} }, created: Date.now() } as any);
