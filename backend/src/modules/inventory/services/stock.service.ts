...
export class StockService {
  ...
  async adjustStock(
    ...
    const result = await runWithIdempotency({
      ...
      work: (tx) => this.applyAdjustStock(dto, companyId, userId, tx),
    });
    return result.body as StockMovementEntity;
  }

  /**
   * F2 — atomic adjustment body. Runs inside the caller's transaction so the
   * IdempotencyRecord reservation, the stock/cost-layer writes, the published
   * event and the audit log commit (or roll back) together.
   */
  private async applyAdjustStock(
    dto: AdjustStockDto,
    companyId: string,
    userId: string,
  ...
  async transferStock(
    ...
    const result = await runWithIdempotency({
      ...
      requestHashPayload: { ...dto, userId },
      status: HttpStatus.CREATED,
      work: (tx) => this.applyTransferStock(dto, companyId, userId, tx),
    });
    return result.body as StockMovementEntity[];
  }

  /**
   * F2 — atomic transfer body. Runs inside the caller's transaction so the
   * reservation, both stock updates and both StockMovement rows (out + in)
   * commit or roll back together — one replay can never create the source or
   * destination movement a second time.
   */
  private async applyTransferStock(
    dto: TransferStockDto,
    companyId: string,
    userId: string,
    tx: Prisma.TransactionClient,
